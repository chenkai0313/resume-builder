    ...
    <div className="bg-white text-gray-800 font-serif p-8 max-w-[210mm] mx-auto">
      <div className="text-center border-b-2 border-gray-400 pb-4 mb-6">
        ...
        <div className="text-sm text-gray-500 mt-1">
          {[
            ...
            p.workMode ? workModeDisplay(p.workMode, lang) : '',
          ].filter(Boolean).join(' | ')}
        </div>
      </div>

      {data.advantages && <div className="mb-5 text-sm italic leading-relaxed text-gray-600" dangerouslySetInnerHTML={{ __html: data.advantages }} />}
            ...
            <div key={exp.id} className="mb-4">
              ...
              <ul className="mt-2 space-y-1">
                {exp.description.filter(Boolean).map((d, i) => (
                  <li key={i} className="text-sm text-gray-600 flex gap-2"><span className="text-gray-400">—</span><span dangerouslySetInnerHTML={{ __html: d }} /></li>
                ))}
              </ul>
            </div>
          ))}
      ...
      {data.skills.length > 0 && (
        <Section title={T.skills}>
          <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-sm text-gray-600">{data.skills.join(' · ')}</div>
        </Section>
      )}
        ...
        <Section title={T.projects}>
          {data.projects.map((proj) => (
            <div key={proj.id} className="mb-3">
              ...
              {proj.techStack.length > 0 && <p className="text-xs text-gray-400 mt-0.5">{proj.techStack.join(' · ')}</p>}
              {proj.description && <div className="text-sm text-gray-600 mt-1" dangerouslySetInnerHTML={{ __html: proj.description }} />}
            </div>
          ))}
        </Section>
      )}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-5">
      <h2 className="text-base font-bold uppercase tracking-wider text-gray-800 mb-2">{title}</h2>
      {children}
    </div>
  )
}
